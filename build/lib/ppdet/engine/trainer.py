...
from ppdet.metrics import (COCOMetric, JDEDetMetric, KeyPointTopDownCOCOEval,
                           KeyPointTopDownMPIIEval, Metric, RBoxMetric,
                           SNIPERCOCOMetric, VOCMetric, WiderFaceMetric,
                           get_infer_results)
...
from .callbacks import (Callback, Checkpointer, ComposeCallback, LogPrinter,
                        SniperProposalsGenerator, VisualDLWriter,
                        WiferFaceEval)
...
class Trainer(object):
    ...
    def train(self, validate=False):
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            model.train()
            iter_tic = time.time()
            for step_id, data in enumerate(self.loader,start=1):
                ...
                if self.cfg.get('fp16', False):
                    ...
                    loss = outputs['loss']
                    avg_loss += loss
                    outputs['avg_loss'] = avg_loss/step_id
                    # model backward
                    loss.backward()
                    self.optimizer.step()
